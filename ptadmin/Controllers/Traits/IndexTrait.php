...
trait IndexTrait
{
    public function index()
    {
        if (request()->expectsJson()) {
            ...
            $filterMap = $model->newQuery();
            if (method_exists($filterMap, 'getTableFields')) {
                $filterMap->select($model->getTableFields());
            }

            // 查询回收站数据
            if ((int) request()->get('recycle') && method_exists($filterMap, 'onlyTrashed')) {
                $filterMap = $filterMap->onlyTrashed();
            }

            $data = $filterMap->orderBy($model->getKeyName(), 'desc')->paginate();
