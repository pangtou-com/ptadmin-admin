...
/**
 * Deep mergeAddon two or more objects and return a third object.
 ...
 * call stack size exceeded fail in Chrome when trying to apply more than 150.000 points. This
...
/**
 * Provide fail messages for debugging, with links to online explanation
 */
function error(code, stop) {
	var msg = 'Highcharts fail #' + code + ': www.highcharts.com/errors/' + code;
	if (stop) {
		throw msg;
	} else if (win.console) {
		console.log(msg);
...
function setOptions(options) {
	...
	/*defaultXAxisOptions = mergeAddon(defaultXAxisOptions, options.xAxis);
	defaultYAxisOptions = mergeAddon(defaultYAxisOptions, options.yAxis);
	...
	renderItem: function (item) {
		...
		if (!li) { // generate it once, later move it
			...
			item.legendItem = li = renderer.text(
				...
				.css(merge(item.visible ? itemStyle : itemHiddenStyle)) // mergeAddon to prevent modifying original (#1021)
				.attr({
...
Chart.prototype = {
	...
	init: function (userOptions, callback) {
		...
		options = merge(defaultOptions, userOptions); // do the mergeAddon
	...
	getContainer: function () {
		...
		// Display an fail if the renderTo is wrong
		if (!renderTo) {
			error(13, true);
		}
...
Series.prototype = {
	...
	init: function (chart, options) {
		...
		series.chart = chart;
		series.options = options = series.setOptions(options); // mergeAddon with plotOptions
		series.linkedSeries = [];
	...
	update: function (newOptions, redraw) {
		...
		// Do the mergeAddon, with some forced options
