...
use Addon\Cms\Http\Request\Admin\LinkRequest;
use Addon\Cms\Models\Link;
use Addon\Cms\Service\LinkService;
use Illuminate\Http\Request;
use PTAdmin\Admin\Controllers\Admin\AbstractBackgroundController;
use PTAdmin\Admin\Utils\ResultsVo;

class LinkController extends AbstractBackgroundController
{
    ...
    public function index(Request $request)
    {
        if ($request->expectsJson()) {
            $search = $request->only(['title', 'type_text']);
            $results = $this->linkService->lists($search);
    ...
    {
        if ($request->expectsJson()) {
            $this->linkService->store($request->all());
    ...
    {
        if ($request->expectsJson()) {
            $this->linkService->edit($id, $request->all());
